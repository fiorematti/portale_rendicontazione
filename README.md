...
## 2.2 Gestione collaborativa con GitHub: Branching strategy, flussi di commit e documentazione tecnica
...
Sul flusso di commit mi tengo leggero ma ordinato: tante PR piccole, titoli chiari, descrizioni che spiegano cosa cambia e perché. I messaggi di commit seguono uno schema descrittivo (es. `fix: gestisci errori 401 in login`) per rendere il log leggibile. Prima di aprire una PR faccio un rebase o un merge da main per ridurre conflitti, e lascio che la pipeline (anche solo i test locali) giri per avere un minimo di qualità automatica.
