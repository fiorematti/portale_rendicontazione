...
Il cuore sta in `src/`. Qui `main.ts` esegue il bootstrap dell’app con `bootstrapApplication(AppComponent, appConfig)`.
...
La cartella `auth/` contiene tutto ciò che riguarda accesso e protezione: `auth.service.ts` incapsula la logica MSAL, `auth.interceptor.ts` aggancia il token alle richieste, `auth.guard.ts` blocca le rotte non autorizzate e `login.ts/html/css` gestisce la pagina di login.

I DTO scambiati con le API sono in `dto/` (ad esempio i modelli per clienti, ordini, automobili, luoghi e la risposta di autenticazione), mentre `mappers/` ospita la logica di trasformazione (`authmapper.ts`).

Le configurazioni di ambiente vivono in `config/env.ts`, con base URL e parametri MSAL centralizzati.
